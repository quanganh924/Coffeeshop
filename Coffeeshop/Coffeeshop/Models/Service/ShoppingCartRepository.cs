...
s.Product.Id == product.Id && s.ShoppingCartId == ShoppingCartId); if (shoppingCartItem == null)
            {
        ...
        public void ClearCart()
        {
            ...
            dbContext.ShoppingCartItems.RemoveRange(cartItems); dbContext.SaveChanges();
        }
...
s.ShoppingCartId == ShoppingCartId).Include(p => p.Product).ToList();
        ...
        public decimal GetShoppingCartTotal()
        {
            var totalCost = dbContext.ShoppingCartItems.Where(s => s.ShoppingCartId == ShoppingCartId)
                .Select(s => s.Product.Price * s.Qty).Sum(); return totalCost;
        }
        ...
        public int RemoveFromCart(Product product)
        {
            ...
            var shoppingCartItem = dbContext.ShoppingCartItems.FirstOrDefault(s =>
s.Product.Id == product.Id && s.ShoppingCartId == ShoppingCartId); var quantity = 0;

            if (shoppingCartItem != null)
            {
                if (shoppingCartItem.Qty > 1)
                {
                    shoppingCartItem.Qty--; quantity = shoppingCartItem.Qty;
                }
                else
                {
            ...
            dbContext.SaveChanges(); return quantity;
